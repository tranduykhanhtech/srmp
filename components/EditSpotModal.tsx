...
import { X, Check, MapPin, Tag, Pencil, Navigation, Compass } from 'lucide-react';
...
interface EditSpotModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (spotId: string, updatedData: {
    name: string;
    address: string;
    category: string;
    note?: string;
...
export default function EditSpotModal({
  ...
  // Sync state with selected spot
  useEffect(() => {
    if (spot && isOpen) {
      setName(spot.name || '');
      setAddress(spot.address || '');
      setNote(spot.note || '');
      setCategory(spot.category || 'Tụ họp');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error editing spot:', err);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end sm:justify-center sm:items-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-150 p-0 sm:p-4">
      <div className="fixed inset-0 -z-10" onClick={() => !isSubmitting && onClose()} />
      ...
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl bg-white border border-neutral-200 text-black shadow-2xl p-5 sm:p-6 space-y-4 max-h-[92vh] flex flex-col pb-safe animate-in slide-in-from-bottom-6 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-neutral-100 pb-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-neutral-900">Chỉnh sửa địa điểm</h2>
              <p className="text-xs text-neutral-500">Cập nhật thông tin quán trên animon</p>
            </div>
          </div>
          <button
            onClick={onClose}
